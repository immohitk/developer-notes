...
Unused CSS consists of rules that are no longer used by the current HTML, components, or application.

As a project changes, old styles may remain in the stylesheet even after the related elements have been removed.

Removing unused CSS helps keep stylesheets smaller, clearer, and easier to maintain.

### How Unused CSS Appears

Unused CSS can accumulate when:

```text
Components are removed
Pages are redesigned
Class names are changed
Old features are replaced
Temporary styles are no longer needed
```
...
If no element uses the `old-banner` class anymore, the rule may be unnecessary.

### Unused CSS Makes Stylesheets Harder to Read

Consider a stylesheet containing:
...
If `.old-sidebar` and `.temporary-button` are no longer used, they add unnecessary code.

A cleaner stylesheet is easier to understand:
...
### Remove Old Styles Carefully

Before deleting a CSS rule, confirm that it is not used.

A class may appear in:
...
For example, a class might be added dynamically:

```javascript
element.classList.add("active");
```

Removing the `.active` CSS rule without checking JavaScript could cause visual problems.

### Review CSS Regularly

Unused styles are easier to manage when they are removed regularly.
...
This prevents old styles from accumulating over time.

### Avoid Keeping Temporary CSS Forever

Temporary styles are sometimes added while debugging or developing a feature.
...
If the rule is no longer needed, it should be removed.

Temporary CSS can make a stylesheet confusing when it remains after its original purpose is forgotten.

### Use Clear Naming and Organization

Clear naming can make unused CSS easier to identify.
...
When a component is removed, related styles can be reviewed together.

Keeping related styles organized also makes cleanup easier.

### Tools Can Help Identify Unused CSS

Development tools and CSS analysis tools can help identify styles that may no longer be used.

However, tool results should be reviewed carefully.
...
Applied during certain states
Generated by JavaScript
```

A style should not be removed automatically without confirming that it is truly unnecessary.

### Remove Unused CSS Summary

```text
Remove Unused CSS
│
├── Keeps stylesheets cleaner
├── Makes CSS easier to read
├── Reduces unnecessary code
├── Makes maintenance easier
├── Prevents old styles from accumulating
└── Requires checking before deletion
```
